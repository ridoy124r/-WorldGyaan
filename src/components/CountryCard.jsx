...
const CountryCard = ({ country }) => {
  const { name, region, population, capital, flags, independent } = country;
  ...
  return (
    <li className="group relative bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-7 shadow-xl transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:bg-white/20">
      ...
      <NavLink to={`/country/${name.common}`}>
        <button className="bg-gray-400 text-gray-200 hover:bg-gray-600 transition ease-in  px-4 py-2 rounded-md mt-2">
          Read More
        </button>
      </NavLink>
    </li>
